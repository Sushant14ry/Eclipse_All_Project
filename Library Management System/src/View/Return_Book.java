...
//			Diplay_Book();
	...
	private JPanel getPanel() {
		if (panel == null) {
			...
			panel.setBounds(631, 70, 479, 429);
			panel.add(getBid());
	...
	private JPanel getPanel_1() {
		if (panel_1 == null) {
			...
			panel_1.setBounds(24, 70, 570, 382);
		...
		if (btnSearch_1 == null) {
			...
			btnSearch_1.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					rbs.getIReturnBookById(search_Student);
						
					Display_Student_Taken_Book(search_Student);
				}
			});
...
//					Insert Date in Database
					...
					if(res) {
						...
						int stdid = Integer.parseInt(sidinput.getText());
						if(std_id == stdid ) {
							
						}else {
							JOptionPane.showMessageDialog(null, "This ID Number of Student is Already Return the Book");
						}
			...
			btnIssue.setBounds(196, 524, 128, 35);
		}
		return btnIssue;
	}
	private JButton getBtnBack_1() {
		if (btnBack_1 == null) {
			...
			btnBack_1.setBounds(368, 524, 91, 35);
		}
		return btnBack_1;
	}
	...
	private JSeparator getSeparator() {
		if (separator == null) {
			...
			separator.setBounds(460, 43, 202, 17);
		}
		return separator;
	}
	private JDateChooser getReturndate() {
		if (returndate == null) {
			...
			returndate.setBounds(197, 470, 263, 29);
		}
		return returndate;
	}
	private JLabel getLblFatherName_1_1_1_1_1() {
		if (lblFatherName_1_1_1_1_1 == null) {
			...
			lblFatherName_1_1_1_1_1.setBounds(46, 474, 141, 23);
	...
	private JTable getTable() {
		if (table == null) {
			table = new JTable();
			table.setBounds(556, 520, 1, 1);
		}
		return table;
	}

	// Fetch Data from Student and Show Which Book he has Taken From Libaray and show in Input Box
	private void Display_Student_Taken_Book(int sid) {
		
		eid = sid;
		Return_Book_Service ibks = new Return_Service_Book_empl();
		Return_book ibm = ibks.getIReturnBookById(sid);
		
		fullnameinput.setText(ibm.getFullname());
		fathernameinput.setText(ibm.getFathername());
		cmbcourse.setSelectedItem(ibm.getCourse());
		cmbbranch.setText(ibm.getBranch());
		cmbyear.setSelectedItem(ibm.getYear());
		cmbsemester.setSelectedItem(ibm.getSemester());
		
		// Fetch Books Code
		
		bid.setText(String.valueOf(ibm.getBid()));
		bookname.setText(ibm.getBookname());
		cmbedition.setSelectedItem(ibm.getBookedition().toString());
		bookpublisher.setText(ibm.getBookpublisher());
		bookprice.setText(String.valueOf(ibm.getBookprice()));
		bookpage.setText(String.valueOf(ibm.getBookpage()));
		bookwritter.setText(ibm.getBookwriter());
		dateIssue.setDate(ibm.getIssue_date());
		
	}
	
	private void Dispay_ReturnBookInTable() {
		
		Return_Book_Service rbs = new Return_Service_Book_empl();
		List<Return_book> rbk_list = rbs.GetAllReturnBook();
		
		DefaultTableModel tmodel = (DefaultTableModel) table.getModel();
		tmodel.setColumnCount(0);
		
		for(Return_book bk : rbk_list) {
			std_id = bk.getSid();
			tmodel.addRow(new Object[] {bk.getSid()});
		}
